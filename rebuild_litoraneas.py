#!/usr/bin/env python3
"""Reconstrói litoraneas.json com municípios defrontantes com o mar (IBGE 2024)."""
...
DATA = ROOT / "data" / "litoraneas.json"
...
def load_coastal_ibge_codes() -> set[str]:
    """Load IBGE codes from the official coastal municipalities XLS."""
    if not XLS_PATH.exists():
        print(f"XLS não encontrado: {XLS_PATH}")
        print("Baixe de: https://geoftp.ibge.gov.br/organizacao_do_territorio/estrutura_territorial/municipios_defrontantes_com_o_mar/2024/Municipios_Defrontantes_com_o_Mar_2024.xls")
        return set()
    if xlrd is None:
        print("xlrd não instalado. Instale com: pip install xlrd")
        return set()
    wb = xlrd.open_workbook(str(XLS_PATH))
    ws = wb.sheet_by_index(0)
    codes = set()
    for i in range(1, ws.nrows):
        cod = str(int(ws.cell_value(i, 0))).zfill(7)
        codes.add(cod)
    return codes


def load_refs(pop: dict[str, int], coastal_codes: set[str]) -> list[dict]:
    rows = []
    with open(REF, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            cod = str(row["cod7"])
            if cod not in coastal_codes:
                continue
            p = pop.get(cod)
            if not p:
                continue
...
def load_homicidios() -> dict[str, dict]:
    ...
    path = CACHE / "homicidios.csv"
    if not path.exists():
        print("homicidios.csv ausente no cache")
        return by_cod
    with open(path, encoding="utf-8", newline="") as f:
        for row in csv.DictReader(f):
            year = int(row["year"])
...
def pad7_from6(refs: list[dict]) -> dict[str, str]:
    return {r["cod"][:6]: r["cod"] for r in refs}
...
def main():
    pop = load_pop()
    coastal_codes = load_coastal_ibge_codes()
    if not coastal_codes:
        print("Nenhum código costeiro carregado. Abortando.")
        return
    print(f"Municípios costeiros IBGE 2024: {len(coastal_codes)}")

    refs = load_refs(pop, coastal_codes)
    print(f"Municípios com dados IDH + pop: {len(refs)}")
    ...
    print(f"Candidatas (≤{MAX_POP} hab.): {len(small)} | médias ≥150k: {len(medium)} | grandes ≥500k: {len(large)}")
    ...
    edu = {}
    seg = {}
    saude_path = CACHE / "munic2021_saude.csv"
    edu_path = CACHE / "munic2021_educacao.csv"
    seg_path = CACHE / "munic2023_seguranca.csv"
    if saude_path.exists():
        saude = load_csv_by_cod(saude_path)
    if edu_path.exists():
        edu = load_csv_by_cod(edu_path)
    if seg_path.exists():
        seg = load_csv_by_cod(seg_path)
    ...
    payload = {
        "meta": {
            "titulo": f"{len(entries)} cidadezinhas litorâneas para dev remoto",
            "criterios": [
            ...
            "distribuicao": {
                "sul_sudeste": reg.get("Sul", 0) + reg.get("Sudeste", 0),
