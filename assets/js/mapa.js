/* Beach Office — Mapa Leaflet, filtros na sidebar e estado compartilhável na URL. */

const REGIOES = ["Nordeste", "Sudeste", "Sul", "Norte"];
...
const FAIXAS_IDH = [
  { max: 0.65, cor: "#1e3a5f", texto: "IDH < 0,650" },
  { max: 0.7, cor: "#0e7490", texto: "0,650 – 0,699" },
  { max: 0.75, cor: "#06b6d4", texto: "0,700 – 0,749" },
  { max: Infinity, cor: "#22d3ee", texto: "≥ 0,750" },
];
...
const SIDEBAR_KEY = "beach-office:filtros-abertos";
...
async function loadCities() {
  ...
  const res = await fetch(`data/litoraneas.json?v=${Date.now()}`, { cache: "no-store" });
